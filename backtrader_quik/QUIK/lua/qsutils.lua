...
local function disconnected()
    ...
    -- A callback created by SetUpdateCallback keeps the Lua DataSource alive.
    -- If the Python peer disappears before a normal unsubscribe, that callback
    -- remains bound to the old socket session and later subscriptions can stop
    -- publishing candles.  Close every orphan before accepting a new client.
    if type(data_sources) == "table" then
        for key, ds in pairs(data_sources) do
            if ds then pcall(ds.Close, ds) end
    ...
    -- Recreate the listening sockets after every Python client session.  Some
    -- QUIK/LuaSocket builds leave the accepted server sockets unusable after
    -- both peer connections are closed, while the Lua variables stay non-nil.
    if response_server then
        pcall(response_server.close, response_server)
        response_server = nil
    end
    if callback_server then
        pcall(callback_server.close, callback_server)
        callback_server = nil
    end
