...
function qsfunctions.get_candles_from_data_source(msg)
	...
	-- QKData registers the live subscription before requesting history.  Reuse
	-- that DataSource: QUIK Junior may leave a second DataSource for the same
	-- instrument/interval permanently empty.
	local ds = data_sources and data_sources[key] or nil
	local owns_ds = false
	local is_error = false
	if ds == nil then
		ds, is_error = create_data_source(msg)
		owns_ds = not is_error and ds ~= nil
	end
	if not is_error then
		--- Датасорс изначально может быть пустым. Никогда не ждём бесконечно:
		--- Python socket_timeout равен 15 секундам.
		local deadline = timemsec() + 10000
		while ds:Size() == 0 and timemsec() < deadline do
			delay(100)
		end
		if ds:Size() == 0 then
			msg.cmd = "lua_data_source_timeout"
			msg.lua_error = "DataSource is empty after 10 seconds for " .. key
			msg.data = {}
			if owns_ds then ds:Close() end
			return msg
		end
...
function qsfunctions.subscribe_to_candles(msg)
	...
	if not is_error then
		local class, sec, interval, param = get_candles_param(msg)
		local key = get_key(class, sec, interval, param)
		-- QUIK 11.x can return a valid but still empty DataSource.  Installing
		-- SetUpdateCallback at size 0 reports success, but that callback may
		-- never be invoked after the history arrives.  Wait for initial data
		-- before binding the callback, using the same bounded wait as history.
		local deadline = timemsec() + 10000
		while ds:Size() == 0 and timemsec() < deadline do
			delay(100)
		end
		...
		callback_statuses[key] = {
			callback_set = false,
			created_at = timemsec(),
			initial_size = ds:Size(),
			callback_invocations = 0,
			published_candles = 0
		}
		local callback_set = ds:SetUpdateCallback(
			function(index)
				local status = callback_statuses[key]
				if status ~= nil then
					status.callback_invocations = status.callback_invocations + 1
					status.last_callback_index = index
					status.last_callback_at = timemsec()
				end
				local ok, err = pcall(data_source_callback, index, class, sec, interval, param)
				if not ok then
					if status ~= nil then status.last_callback_error = tostring(err) end
					log("DataSource callback error for " .. key .. ": " .. tostring(err), 3)
				end
			end)
		callback_statuses[key].callback_set = callback_set == true
		if not callback_set then
			msg.cmd = "lua_set_update_callback_error"
			msg.lua_error = "SetUpdateCallback returned false for " .. key
			callback_statuses[key].last_callback_error = msg.lua_error
			pcall(ds.Close, ds)
...
function data_source_callback(index, class, sec, interval, param)
	...
	if index ~= last_indexes[key] then
		...
		local candle = fetch_candle(ds, index - 1)
		candle.sec = sec
		candle.class = class
		candle.interval = interval
        ...
        sendCallback(msg)
		local status = callback_statuses[key]
		if status ~= nil then status.published_candles = status.published_candles + 1 end
	end
end

--- Return diagnostic state for the candle callback without changing it.
function qsfunctions.get_candle_subscription_status(msg)
	local class, sec, interval, param = get_candles_param(msg)
	local key = get_key(class, sec, interval, param)
	local status = callback_statuses[key]
	if status == nil then
		msg.data = { key = key, subscribed = false }
		return msg
	end
	local result = {}
	for k, v in pairs(status) do result[k] = v end
	result.key = key
	result.subscribed = data_sources[key] ~= nil
	if data_sources[key] ~= nil then result.current_size = data_sources[key]:Size() end
	result.last_index = last_indexes[key]
	msg.data = result
	return msg
end
...
function qsfunctions.unsubscribe_from_candles(msg)
	local class, sec, interval, param = get_candles_param(msg)
	local key = get_key(class, sec, interval, param)
	if data_sources[key] ~= nil then pcall(data_sources[key].Close, data_sources[key]) end
	data_sources[key] = nil
	last_indexes[key] = nil
	callback_statuses[key] = nil
	return msg
end
