...
Первый стабильный релиз и первая публикация на PyPI.

- версия переведена из предрелизной `1.0.0a5` в `1.0.0`: пакет ставится обычным
  `pip install backtrader-quik`, без флага `--pre`;
- метаданные подготовлены к публикации: лицензионные файлы `LICENSE.md` и
  `THIRD_PARTY_NOTICES.md` включены в дистрибутивы, добавлены Homepage и
  Changelog URL, классификаторы аудитории и лицензии;
- `MANIFEST.in` исправлен: в sdist добавлены `CONTRIBUTING.md`, `SECURITY.md` и
  `pytest.ini`, убраны несуществующие пути;
- ссылки в README сделаны абсолютными, чтобы страница пакета на PyPI
  отображалась корректно;
- добавлена публикация по тегу `v*` через PyPI Trusted Publishing;
- функциональных изменений торгового кода относительно `1.0.0a5` нет.
